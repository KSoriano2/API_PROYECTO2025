...
export const postConsultorio = async(req, res)=>{
    try{
        ...
        const {nombre_consultorio,ubicacion_consultorio, id_inst_esp} =req.body
        const [result] = await conmysql.query(' INSERT INTO CONSULTORIOS (NOMBRE_CONSULTORIO,UBICACION_CONSULTORIO,ESTADO_CONSULTORIO, ID_INST_ESP) VALUES(?,?,?,?)', 
        [nombre_consultorio,ubicacion_consultorio,estado_consultorio, id_inst_esp])
...
export const putConsultorio=async(req,res)=>{
    try{
        const {id} = req.params
        const {nombre_consultorio,ubicacion_consultorio,estado_consultorio, id_inst_esp}=req.body
        
        const [result] = await conmysql.query(
            'UPDATE CONSULTORIOS SET NOMBRE_CONSULTORIO=?,UBICACION_CONSULTORIO=?,ESTADO_CONSULTORIO=?, ID_INST_ESP=? WHERE ID_CONSULTORIO=?',
            [nombre_consultorio,ubicacion_consultorio,estado_consultorio,id_inst_esp,id])
...
export const patchConsultorio=async(req,res)=>{
    try{
        ...
        const [result] = await conmysql.query(
            'UPDATE INSTITUCION_ESPECIALIDAD SET NOMBRE_CONSULTORIO=IFNULL(?, NOMBRE_CONSULTORIO),UBICACION_CONSULTORIO=IFNULL(?, UBICACION_CONSULTORIO),ESTADO_CONSULTORIO=IFNULL(?, ESTADO_CONSULTORIO), ID_INST_ESP=IFNULL(?, ID_INST_ESP) WHERE ID_CONSULTORIO=?',
            [nombre_consultorio,ubicacion_consultorio,estado_consultorio, id_inst_esp,id])
